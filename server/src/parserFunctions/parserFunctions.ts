...
import { CompletionItem, CompletionItemKind, InsertTextFormat, Position, TextEdit } from 'vscode-languageserver';
		...
		xml2js.parseString(wholeFile, (err, result) => {
			...
			while (result.root.snippet[i]) {
				...
				if(result.root.snippet[i].docu) {
					...
					docu = result.root.snippet[i].text[0];
				}
				let item :CompletionItem = {
					...
					insertText: result.root.snippet[i].keyword[0]
				}
